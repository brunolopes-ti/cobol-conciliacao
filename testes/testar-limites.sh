...
def testar(
    ...
    *,
    ...
    try:
        ...
        assert p.returncode == codigo, (
            f'Codigo {p.returncode}; esperado {codigo}'
        )

        for mensagem in mensagens:
            assert mensagem in saida, (
                f'Mensagem ausente: {mensagem}'
            )

        for mensagem in ausentes:
            assert mensagem not in saida, (
                f'Mensagem indevida: {mensagem}'
            )

        assert (
            caso / 'dados/esperados.csv'
        ).read_bytes() == esperados

        assert (
            caso / 'dados/recebidos.csv'
        ).read_bytes() == recebidos

        if programa != 'conciliacao' or codigo != 0:
            assert (
                caso / 'relatorio.txt'
            ).read_bytes() == anterior, 'Relatorio alterado'

        assert not list(
            caso.glob('.conciliacao-*.tmp')
        ), 'Temporario restante'
...
for programa in ('leitor', 'conciliacao'):
    for nome, texto, codigo, mensagem in (
        (
            'ID ASCII de 50 caracteres',
            'A' * 50 + ';100.00\n',
            0,
            'Pagamento: ' + 'A' * 50
        ),
        (
            'ID ASCII de 51 caracteres',
            'A' * 51 + ';100.00\n',
            1,
            limite_id
        ),
        (
            'ID acentuado de 50 caracteres',
            'Á' * 50 + ';100.00\n',
            0,
            'Pagamento: ' + 'Á' * 50
        ),
        (
            'ID acentuado de 51 caracteres',
            'Á' * 51 + ';100.00\n',
            1,
            limite_id
        ),
        (
            'ID UTF-8 de quatro bytes',
            '😀' * 50 + ';100.00\n',
            0,
            'Pagamento: ' + '😀' * 50
        ),
        (
            'ID com espaco inicial',
            ' P001;100.00\n',
            1,
        ...
        (
            'linha exata de 256 bytes',
            'P001;100.00' + ' ' * 245 + '\n',
            0,
            'Pagamento: P001'
        ),
        (
            '257 bytes com espacos finais',
            'P001;100.00' + ' ' * 246 + '\n',
            1,
            limite_linha
        ),
        (
            'excesso escondido apos 5000 espacos',
            'P001;100.00' + ' ' * 5000 + 'abc\n',
            1,
            limite_linha
        ),
        (
        ...
        testar(
            ...
            codigo,
            [mensagem],
            esperados=texto.encode()
        )

    testar(
        f'{programa}: UTF-8 invalido',
        programa,
        1,
        [controle],
        esperados=b'P\xff;100.00\n'
    )

# O leitor deve recuperar a fronteira da linha, sem inventar novos registros.
testar(
    'leitor: continua depois de linha muito longa',
    'leitor',
    1,
    [
        'Erro na linha 1:',
        'Pagamento: P002',
        'Total de registros lidos: 2',
        'Registros validos: 1',
...
testar(
    'conciliacao: recebido longo preserva relatorio',
    'conciliacao',
    1,
    [limite_linha],
    recebidos=(
...
testar(
    'operador com 40 bytes',
    'ola',
    0,
    ['Operador: ' + 'A' * 40, status],
    entrada=b'A' * 40 + b'\n100\n100\n'
)

testar(
    'operador longo pede novamente',
    'ola',
    0,
...
testar(
    'operador UTF-8 sem corte parcial',
    'ola',
    0,
...
testar(
    'esperado com excesso oculto e nova tentativa',
    'ola',
    0,
...
testar(
    'recebido com excesso oculto e nova tentativa',
    'ola',
    0,
...
testar(
    'EOF depois de entrada rejeitada',
    'ola',
    1,
    [
        limite_terminal,
...
testar(
    'valor com NUL nao vira numero valido',
    'ola',
    0,
...
testar(
    'interativo aceita ultima linha sem quebra',
    'ola',
    0,
    [status],
    entrada=b'Bruno\n100\n100'
...
testar(
    'argumento longo com excesso oculto',
    'conciliacao',
    2,
    ['Erro: caminho excede 256 posicoes.'],
    argumentos=(
        'dados/esperados.csv' + ' ' * 5000 + 'x',
        'dados/recebidos.csv',
        'relatorio.txt',
        'resultado.tsv'
    )
...
testar(
    'argumento com espaco final rejeitado',
    'conciliacao',
    2,
