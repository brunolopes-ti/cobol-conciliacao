...
executar() {
    ...
    (
        ...
        if [ "$modo" = escrita ]; then
            ...
            exec env LD_PRELOAD="$biblioteca" "$pasta_teste/conciliacao" "$@"
        else
    ...
    if [ "$esperado" -ne 0 ] && grep -Fxq 'Conferencia concluida.' "$caso/saida.txt"; then
        echo 'Sucesso anunciado apesar da falha.'
        falhou=1
    fi
    if [ "$esperado" -eq 0 ]; then
        grep -Fxq 'Conferencia concluida.' "$caso/relatorio.txt" || falhou=1
        grep -Fxq 'Saldo global: -20.00' "$caso/relatorio.txt" || falhou=1
        cmp -s "$caso/antigo-via-link" "$caso/copia-relatorio" || falhou=1
    ...
    if [ -L "$caso/link-quebrado" ] && [ -e "$caso/nao-existe" ]; then
        falhou=1
    fi
    if [ "$falhou" -eq 0 ]; then
        echo "PASSOU: $nome"
...
executar 'alias com ponto nos esperados' 2 \
    dados/esperados.csv dados/recebidos.csv ./dados/esperados.csv

preparar
executar 'alias com dois pontos nos recebidos' 2 \
    dados/esperados.csv dados/recebidos.csv subpasta/../dados/recebidos.csv

preparar
executar 'caminho absoluto da entrada' 2 \
    dados/esperados.csv dados/recebidos.csv "$caso/dados/esperados.csv"

preparar
ln -s dados/esperados.csv "$caso/link-saida"
executar 'link simbolico para esperados' 2 \
    dados/esperados.csv dados/recebidos.csv link-saida
...
executar 'alias por diretorio simbolico' 2 \
    dados/esperados.csv dados/recebidos.csv atalho/esperados.csv
...
executar 'link simbolico quebrado rejeitado' 1 \
    dados/esperados.csv dados/recebidos.csv link-quebrado
